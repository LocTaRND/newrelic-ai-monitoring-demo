...
# K3s + Helm Setup Script for Ubuntu
# This script installs and configures K3s and Helm on Ubuntu
...
install_docker() {
    if ! command -v docker &> /dev/null; then
        ...
        # Download and install Docker using convenience script
...
install_k3s() {    
    ...
    curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION="v1.30.8+k3s1" INSTALL_K3S_EXEC="--write-kubeconfig-mode 644" sh -
...
post_install_info() {
    ...
    echo "  curl -sfL https://get.k3s.io | K3S_URL=https://$(hostname -I | awk '{print $1}'):6443 K3S_TOKEN=$(sudo cat /var/lib/rancher/k3s/server/node-token) sh -"
    echo
}
